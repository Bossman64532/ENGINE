...
def load_server_endpoint() -> tuple[str, int]:
    ...
    if config_path.exists():
        ...
        endpoint = config.get("proxy_hostname") or f"{config.get('server', '178.128.190.144')}:{config.get('port', 8443)}"
...
# Certificate paths
TLS_DIR = Path(os.getenv("IEEE2030_5_TLS_DIR", "~/Documents/Cloudtls")).expanduser()
