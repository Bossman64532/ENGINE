...
ALL communication is over HTTPS with mutual TLS.
...
CONFIG_PATH = Path(os.environ.get("IEEE2030_5_CONFIG", "config.yml"))
...
CONFIG = _load_config()
TLS_REPOSITORY = Path(CONFIG.get("tls_repository", "~/tls")).expanduser()

# Certificate paths
CA   = os.environ.get("IEEE2030_5_CA", str(TLS_REPOSITORY / "certs/ca.crt"))
CERT = os.environ.get("IEEE2030_5_CERT", str(TLS_REPOSITORY / "combined/dev1-combined.pem"))
KEY  = os.environ.get("IEEE2030_5_KEY", CERT)

CONNECT_TARGET = "server"

if CONFIG.get("proxy_enabled") and CONFIG.get("proxy_hostname"):
    _config_host, _config_port = _split_host_port(CONFIG["proxy_hostname"], 8443)
    CONNECT_TARGET = "proxy"
else:
    _config_host = CONFIG.get("server", "localhost")
    _config_port = CONFIG.get("port", 8443)

SERVER_HOST = os.environ.get("IEEE2030_5_HOST", _config_host)
HTTPS_PORT  = int(os.environ.get("IEEE2030_5_PORT", _config_port))
...
def main():
    print("\n" + "=" * 60)
    print(f"IEEE 2030.5 CLIENT  –  HTTPS {SERVER_HOST}:{HTTPS_PORT} ({CONNECT_TARGET}), mutual TLS")
    print("=" * 60)
